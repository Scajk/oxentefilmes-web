...
export default function MovieGrid() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const allIndividualCategories = movieData.flatMap(movie => 
        movie.genre ? movie.genre.split(',').map(g => g.trim()) : []
      );
      const uniqueCategories = Array.from(new Set(allIndividualCategories));
  ...
  const filteredMovies =
    selectedCategory === "all"
      ? movies
      : movies.filter((movie) => movie.category && movie.category.includes(selectedCategory));
